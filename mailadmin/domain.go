...
func GetDomain(domain_name string) (Domain, error) {
	var dom Domain
	row := Db.QueryRow("SELECT domain, description, aliases, mailboxes, maxquota, quota, transport, backupmx, created, modified, active FROM domain where domain = ? ", domain_name)
